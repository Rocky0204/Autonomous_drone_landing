...
class RedMarkerLanding(Node):
    def __init__(self):
        ...
        self.square_step = 1
        self.direction = [(1, 0), (0, 1), (-1, 0), (0, -1)]  # Right, Up, Left, Down
        self.current_dir = 0
        self.move_count = 0
        self.side_length = 1
    ...
    def start_square_spiral_search(self):
        ...
        self.spiral_timer = self.create_timer(1.0, self.square_spiral_move)
        self.get_logger().info("Marker not detected. Starting square spiral search...")

    def square_spiral_move(self):
        """ Moves the drone in a square spiral pattern. """
        if not self.searching_marker:
            return

        twist = Twist()
        dx, dy = self.direction[self.current_dir]
        twist.linear.x = dx * self.square_step
        twist.linear.y = dy * self.square_step

        self.cmd_vel_pub.publish(twist)

        self.move_count += 1
        if self.move_count == self.side_length:
            self.move_count = 0
    ...
    def align_and_land(self, offset_x, offset_y):
        ...
        if abs(offset_x) > self.center_tolerance:
            twist.linear.y = -0.2 if offset_x > 0 else 0.2
            move_x = True

        if abs(offset_y) > self.center_tolerance:
            twist.linear.x = -0.2 if offset_y > 0 else 0.2
            move_y = True
