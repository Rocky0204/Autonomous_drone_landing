...
class DroneObject(Node):
    def __init__(self, node_name: str = "drone_node"):
        ...
        self.pubVelMode = self.create_publisher(Bool, 'drone/dronevel_mode', 1024)

        # Subscribers
        self.sub_sonar = self.create_subscription(Range, 'drone/sonar', self.cb_sonar, 1024)
        self.sub_imu = self.create_subscription(Range, 'drone/imu', self.cb_imu, 1024)
        ...
        self.sub_gt_pose = self.create_subscription(Pose, 'drone/gt_pose', self.cb_gt_pose, 1024)
        self.sub_state = self.create_subscription(Int8, 'drone/state', self.cb_state, 1024)
        self.sub_cmd_mode = self.create_subscription(String, 'drone/cmd_mode', self.cb_cmd_mode, 1024)
